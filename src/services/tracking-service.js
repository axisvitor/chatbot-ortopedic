...
class TrackingService {
    ...
    async updateAllTrackingStatus() {
        try {
            console.log('🔄 [Tracking] Iniciando atualização em massa dos status de rastreamento');
            ...
            if (!keys || keys.length === 0) {
                console.log('ℹ️ [Tracking] Nenhum código de rastreio encontrado para atualizar');
                return;
            }

            console.log(`📦 [Tracking] Atualizando ${keys.length} códigos de rastreio`);
            ...
            for (let i = 0; i < keys.length; i += batchSize) {
                ...
                if (trackingNumbers.length > 0) {
                    await Promise.all(trackingNumbers.map(async (trackingNumber) => {
                        try {
                            ...
                            await this.getTrackingInfo(trackingNumber, true);

                            // Verifica se foi entregue e atualiza na Nuvemshop
                            await this._updateNuvemshopOrderStatus(trackingNumber);

                            // Verifica se tem taxação e notifica
                            const trackingData = await this._getFromCache(trackingNumber);
                            if (trackingData) {
                                await this._handleTaxationEvent(trackingNumber, trackingData);
                            }
                        } catch (error) {
                            console.error('❌ [Tracking] Erro ao atualizar rastreio:', {
                                trackingNumber,
                                erro: error.message
                            });
                            // Não propaga o erro para não interromper o lote
                        }
                    }));
                ...
                console.log(`✅ [Tracking] Processados ${Math.min(i + batchSize, keys.length)}/${keys.length} códigos`);
            }
            
            console.log('🎉 [Tracking] Atualização em massa concluída com sucesso');
            
        } catch (error) {
            console.error('❌ [Tracking] Erro ao atualizar status em massa:', {
                erro: error.message,
                stack: error.stack
            });
            throw error;
